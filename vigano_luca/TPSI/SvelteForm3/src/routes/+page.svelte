...
<form method="POST">
    ...
    <div>
        ...
        <p class={error && nome?.lenght == 0? '': 'hidden'}>NOME INVALIDO</p>
    </div>
    <div><label for="cogome">Cognome</label></div>
    <div>
        <input type="text" id="cognome" name="cognome" size="40" bind:value={cognome}>
        <p class={error && cognome?.lenght == 0? '': 'hidden'}>COGNOME INVALIDO</p>
    </div>
    <div><label for="eta">Età</label></div>
    <div>
        <input type="number" id="eta" name="eta" min="15">
        <p class={error && eta?.lenght == 0? '': 'hidden'}>ETÀ INVALIDA</p>
    </div>
